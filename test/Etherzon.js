...
describe("Etherzon", () => {
    ...
    describe("Deployment", () => {
        it("Sets the owner", async () => {
            expect(await etherzon.owner()).to.equal(deployer.address)
        })
    })
    ...
    describe("Listing", () => {
        ...
        beforeEach(async () => {
            transaction = await etherzon.connect(deployer).list(
                ID,
                NAME,
                CATEGORY,
                IMAGE,
        ...
        it("Returns the item attributes", async () => {
            const item = await etherzon.items(ID)
        ...
        it("Emits List event", () => {
            expect(transaction).to.emit(etherzon, "List")
        })
    ...
    describe("Buying", () => {
        ...
        beforeEach(async () => {
            // List an item
            transaction = await etherzon.connect(deployer).list(ID, NAME, CATEGORY, IMAGE, COST, RATING, STOCK)
            await transaction.wait()
    ...
    describe("Withdrawing", () => {
        ...
        beforeEach(async () => {
          // List a item
          let transaction = await etherzon.connect(deployer).list(ID, NAME, CATEGORY, IMAGE, COST, RATING, STOCK)
          await transaction.wait()
